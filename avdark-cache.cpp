...
int LRU(avdark_cache_t *self, int index)
{
	...
	for(int i = index; i < lines; i += self->number_of_sets)
	{
		...
		{
			ref_timestamp = self->lines[i].timestamp;
			new_index = i;
		}
	}
	return new_index;
...
avdc_access(avdark_cache_t *self, avdc_pa_t pa, avdc_access_type_t type)
{
		...
		int assocciativity = (int) self->assoc;
		int hits[assocciativity] = {-1};
		bool hit_in_a_set;
		
		for(int i = 0; i < assocciativity; i++)
		{
			hits[i] = self->lines[index].valid && self->lines[index].tag == tag;
			index += self->number_of_sets;
		}
		...
		if (!hit_in_a_set) {
				if (assocciativity != 1)
				{
					index = LRU(self, index);
				}
				self->lines[index].valid = 1;
				self->lines[index].tag = tag;
				self->lines[index].timestamp = timestamp; //set time stamp to use in LRU
		}
	
        switch (type) {
        case AVDC_READ: /* Read accesses */
                avdc_dbg_log(self, "read: pa: 0x%.16lx, tag: 0x%.16lx, index: %d, hit: %d\n",
                             (unsigned long)pa, (unsigned long)tag, index, hit_in_a_set);
